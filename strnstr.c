#include "libft.h"
// Localiza a substring 'needle' nos primeiros 'len' caracteres da string 'haystack'.
...
int main(void)
{
	...
	printf("3. Resultado: %s\n", ft_strnstr(haystack, needle3, 25)); // deve dar NULL

	// needle vazia
	printf("4. Resultado: %s\n", ft_strnstr(haystack, needle4, 10)); // deve retornar haystack

	// needle inexistente
	printf("5. Resultado: %s\n", ft_strnstr(haystack, needle5, 50)); // deve dar NULL

	// len menor que posição de needle
	printf("6. Resultado: %s\n", ft_strnstr(haystack, needle2, 10)); // deve dar NULL
